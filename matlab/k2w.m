...
%
% Dispersion relation:
%    w     = sqrt(g*K+0.074*K^3/1000)   (  0 <   w   < inf)
...
if any(ix)
%   w(ix)=ku(ix)+ku2(ix)+sqrt(g*k(ix).*tanh(k(ix)*h));  % w = sqrt(g*K*tanh(K*h))   (  0 <   w   < inf)
  w(ix)=ku(ix)+ku2(ix)+sqrt(9.8*k(ix)+0.074*k(ix).^3/1000); % w = sqrt(g*k+0.074*k^3/1000)   (  0 <   w   < inf)
end
iy=find(w<0);
if any(iy)
